...
Skills and rules carry namespaced tags in frontmatter `metadata`
for filtering and discovery. Tags use `namespace:value` format:

- `category:` — what the artifact does (e.g., `documentation`,
  `testing`, `workflow`)
- `language:` — programming language if applicable (e.g., `go`,
  `bash`)
- `tool:` — external tool involved (e.g., `git`, `makefile`)
...
Tags are auto-generated by the ryu-release skill during each
release and presented to the user for review before committing.
